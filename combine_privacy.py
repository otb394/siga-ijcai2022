...
step_label = 'Step' 
avg_privacy = 'Avg privacy score in Window'
...
for df in dfs:
    if df is not None and avg_privacy not in df:
        df[avg_privacy] = 0

dfs = [x[[step_label, avg_privacy]] if x is not None else None for x in dfs]
...
title = 'Average Privacy for ratio ' + ':'.join(ratio) +  ' (Perfect:Selfish:Generous)'


fig, ax = plt.subplots()
ax.set(xlabel = 'Steps', ylabel = avg_privacy, title = title)

for i in range(no_of_simulations):
    df = dfs[i]
    sim_id = simulations[i]
    if (df is not None):
        ax.plot(df[step_label], df[avg_privacy], color = colors[sim_id - 1], label = labels[sim_id - 1])
